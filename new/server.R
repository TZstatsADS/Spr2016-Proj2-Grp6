...
#       return(t)
#     }})
  drawvalue <- reactive({if (input$offense == ''){
    print("line 30")
    t <- filter(vc, hour==input$hour)
    return(t)}
    else{
      print(input$offense)
    t <- filter(vc, Offense == input$offense,hour==input$hour)
    return(t)
  }})
...
#       }    
    ...
    pal <- colorFactor(palette()[-1], levels(vc$Offense))
    # radius <- draw[[sizeBy]] / 9 * 250 + 30
    # colorData <- draw[["Offense"]]
    radius <-  50
    # pal <- colorBin(rev(heat.colors(10))[-1][-1][-1],levels(colorData),7)
    # pal <- colorFactor("Set1",draw[["Offense"]])
...
#       addLegend("bottomleft", pal=pal, values=levels(colorData),# title=colorBy,
#                 layerId="colorLegend")
  })
